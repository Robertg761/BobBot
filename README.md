...
- **Automations**: Hermes cron jobs, which are how bots reach out proactively. Create, edit, pause, run now, inspect runs. The default delivery is the bot's own chat (`bot-chat:<profile>`), so the result shows up in that conversation and notifies you; combine with `ntfy` for a push even when the phone is off the socket, or use Telegram for jobs other people read.
